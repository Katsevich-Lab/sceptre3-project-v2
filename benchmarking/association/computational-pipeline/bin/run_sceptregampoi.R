#!/usr/bin/env Rscript
# Wrapper script for sceptregampoi computational benchmarking
# (sceptre fork: jdeu1023/sceptre@glmgampoi-experiment)
#
...
#
# Outputs:
# - association_computational_sceptregampoi.csv: Discovery analysis results

args <- commandArgs(trailingOnly = TRUE)
dataset_dir <- args[1]
dataset_id <- args[2]

cat("Running sceptregampoi computational benchmarking\n")
...
# Very permissive QC for computational benchmarking
...
write.csv(results, "association_computational_sceptregampoi.csv", row.names = FALSE)

cat("Sceptregampoi computational benchmarking complete!\n")
cat("Results saved to: association_computational_sceptregampoi.csv\n")
