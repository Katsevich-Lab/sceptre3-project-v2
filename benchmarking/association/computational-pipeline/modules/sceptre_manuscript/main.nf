...
process SCEPTRE_MANUSCRIPT_COMPUTATIONAL {
  tag "${dataset_id}"
  ...
  output:
  tuple val(dataset_id), val(method), path("association_computational_sceptre_manuscript.csv"), emit: results

  publishDir "${outdir}",
             mode: 'copy',
             saveAs: { "association_computational_sceptre_manuscript_${dataset_id}.csv" }
  ...
  # The image installs the manuscript package as the sole `sceptre`, which the script
  # loads via library(sceptre). --vanilla avoids reading host/user profiles or history.
