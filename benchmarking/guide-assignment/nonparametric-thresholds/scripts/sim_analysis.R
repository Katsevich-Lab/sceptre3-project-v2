...
# Model B = 17 data-derived regimes (one per real dataset; the systematic sweep).
...
# ---- overall ranking (all 89 datasets) --------------------------------------
...
cat("=== overall method ranking (mean over all 89 datasets) ===\n"); print(as.data.frame(overall), row.names=FALSE)

# =============================================================================
# MODEL B: the 17 data-derived regimes, ordered easy -> hard by mode separation
# =============================================================================
B <- s %>% filter(model=="B") %>% left_join(reg, by=c("regime"))
reg_order <- reg %>% arrange(desc(separation)) %>% pull(regime)       # easy (high sep) -> hard
B$regime <- factor(B$regime, levels=rev(reg_order))                  # hard -> easy on x
Bagg <- B %>% group_by(regime, separation, family, method) %>%
  summarise(jaccard=mean(jaccard,na.rm=T), FDR=mean(fdr_pooled,na.rm=T), .groups="drop")
...
f_sweep <- ggplot(sweep, aes(regime, value, colour=method, group=method)) +
  ...
  geom_hline(data=data.frame(metric=factor("FDR",levels=c("jaccard","FDR"))), aes(yintercept=0.05),
             linetype="dashed", colour="grey40") +
  facet_wrap(~metric, ncol=1, scales="free_y") + scale_colour_manual(values=mcol) +
  labs(title="Method performance across 17 data-derived regimes (one per real dataset)",
       subtitle="regimes ordered hard -> easy by mode separation (left = cd8/gasperini, right = replogle)",
       x=NULL, y=NULL) +
  theme_bw(base_size=9) + theme(axis.text.x=element_text(angle=40,hjust=1), legend.position="top",
                                legend.title=element_blank())
ggsave(file.path(OUT,"fig_regime_sweep.png"), f_sweep, width=11, height=7, dpi=140)

# heatmap: method x regime, fill = Jaccard
hm <- B %>% group_by(regime, method) %>% summarise(jaccard=mean(jaccard,na.rm=T), .groups="drop")
f_hm <- ggplot(hm, aes(regime, method, fill=jaccard)) + geom_tile() +
  ...
  labs(title="Jaccard by method x regime (17 real-data-derived regimes)", x=NULL, y=NULL) +
...
# regime-level table for the doc
regtab <- B %>% group_by(regime, separation, mu_pert.x, moi.x, target_pct3, sim_pct3) %>%
  summarise(best = ord[which.max(tapply(jaccard, method, mean, na.rm=TRUE))],
            depth_fix=round(mean(jaccard[method=="depth_fix"],na.rm=T),2),
            thresh3=round(mean(jaccard[method=="thresh3"],na.rm=T),2), .groups="drop") %>%
...
af <- A %>% group_by(chemistry, family, method) %>% summarise(FDR=mean(fdr_pooled,na.rm=T), .groups="drop")
f_fdr <- ggplot(af, aes(method, FDR, fill=family)) + geom_col() +
  ...
  labs(title="Model A (real barnyard ambient): realized FDR (dashed = nominal 0.05)", y="mean FDR", x=NULL) +
  theme_bw(base_size=10) + theme(axis.text.x=element_text(angle=45,hjust=1), legend.position="top", legend.title=element_blank())
...
ap <- A %>% filter(chemistry=="direct-capture") %>% group_by(method, purity) %>%
  summarise(FDR=mean(fdr_pooled,na.rm=T), .groups="drop") %>% mutate(purity=factor(paste0("purity ",purity)))
f_pur <- ggplot(ap, aes(method, FDR, fill=purity)) + geom_col(position=position_dodge(0.8), width=0.7) +
  ...
  labs(title="Model A purity sensitivity (direct-capture): FDR at 0.90 vs 0.99",
       subtitle="the doublet false-positive floor at 0.90 collapses at 0.99 for calibrated methods", y="mean FDR", x=NULL) +
  theme_bw(base_size=10) + theme(axis.text.x=element_text(angle=45,hjust=1), legend.position="top", legend.title=element_blank())
ggsave(file.path(OUT,"fig_purity.png"), f_pur, width=9, height=4.5, dpi=140)

# depth_fix vs fishash by MOI (the high-MOI masking story)
moicmp <- s %>% filter(method %in% c("depth_fix","fishash"), model=="A") %>%
  group_by(moi_level=factor(moi_level,levels=c("low","high","vhigh")), method) %>%
  summarise(jaccard=round(mean(jaccard,na.rm=T),3), recall=round(mean(recall,na.rm=T),3), .groups="drop")
write.csv(moicmp, file.path(OUT,"depthfix_vs_fishash_moi.csv"), row.names=FALSE)

cat("\nwrote method_overall.csv, regime_method_table.csv, depthfix_vs_fishash_moi.csv\n")
