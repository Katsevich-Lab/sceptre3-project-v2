...
png(file.path(OUT, "dose_response.png"), width = 1080, height = 680, res = 140)
par(mar = c(4.8, 5.2, 0.8, 1.2), mgp = c(3.2, 0.9, 0), cex.axis = 1.3, cex.lab = 1.32)
...
text(xs, ys, sprintf("%.2f", ys), pos = 3, cex = 1.05, col = "firebrick")
points(max(xs) * 30, sig, pch = 17, col = "steelblue4", cex = 1.8)
text(max(xs) * 30, sig, sprintf("integration\nmode %.2f", sig), pos = 3, cex = 1.15, col = "steelblue4")
dev.off()
