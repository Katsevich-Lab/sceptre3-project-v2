...
## NOTE: barnyard_benchmark.png (the single-mean bar, pB) is no longer used in the
## collaborator writeup — the per-sample Table-2 barplot (barnyard_table2_barplot.png)
## superseded it. Block retained (commented out) for provenance; the fap/fa reads above
## still feed the cat() summary below.
...
pS <- ggplot(ag, aes(moi, recall, colour = method)) +
  ...
  geom_text(data = lab, aes(label = sprintf("%.2f", recall)), hjust = -0.3, size = 5, show.legend = FALSE) +
  scale_colour_manual(values = c("fishash" = "#0072B2", "fishash+" = "#D55E00")) +
  scale_x_continuous(breaks = c(0.3, 1, 3, 5), limits = c(0.2, 5.6)) +
  labs(x = "MOI (mean guides per cell)", y = "recall of true (guide, cell) integrations", colour = NULL) +
  theme_bw(base_size = 16) + theme(legend.position = c(0.14, 0.22),
...
cat("wrote sim_highmoi_recall.png\n")
