...
::: {.callout-tip}
## Companion documents
For the **conceptual/statistical account** of the method landscape — the unifying framework and the
sharper "three background cells" story — see **`literature_review.qmd`** (the canonical synthesis;
this report's framework and failure-mode sections are a condensed pointer to it). For the **simulation
apparatus and sim evidence**, see **`simulation_framework_report.qmd`** (which supersedes the
"comprehensive sim" referenced in §Benchmarks below). This report is the **evidence + recommendation**.
:::

# Executive summary {-}

**Recommendation.** Build on **fishash's contingency core** (a one-sided Fisher/hypergeometric test on
a *noise-decontaminated* 2×2 table + the Guo–Sarkar block FDR), and **decontaminate the *cell* margin**
too — use each cell's *ambient depth* as the test's exposure instead of its raw library size. The
ambient-depth idea is **not new to us: CLEANSER already decontaminates the cell margin** (its
sub-threshold sum $L_i$ from ≤2-count guides). fishash, however, does *not* — it uses the raw library
size for the focal cell's draws (verified from its Eq 7 and from our reproduction, which matches fishash
exactly with `cell_margin="observed"`). **Our contribution is the *unification*:** bringing both
corrections — fishash's guide-side Simpson correction *and* an ambient (decontaminated) cell margin —
into a single transparent contingency test. It recovers recall in the high-MOI / weak-signal regimes
where fishash is over-conservative while **tying fishash on real data**.

**The five load-bearing findings:**

1. **One framework.** Every ambient-aware method tests a count against a rank-1 ambient null
   $\lambda_{g,c}=\gamma_g\kappa_c$ (guide ambient share × cell ambient depth), estimated from a matrix
   that contains the very signal being detected. On the 2×2 table, signal leaks into the three
   *background* cells, giving two failure modes (full account in `literature_review`): **Simpson's
   paradox** (other-cells contamination → false positives → a *precision* loss, fixed by fishash) and
   **self-masking** (the focal cell's own signal → co-occurring guides masked → a *recall* loss at high
   MOI, fixed by the cell-margin decontamination). Overdispersion is a lesser third axis (§below).
...
5. **The depth fix's one apparent weakness was a simulation artifact.** It looked bad at strong signal on
   ...
   de-doubleted barnyard ambient is Poisson (bootstrap-GOF-indistinguishable from Poisson) in both
   chemistries — CROP-seq directly, and
   direct-capture once a gRNA-channel purity cut removes doublets (its raw per-guide overdispersion is
   doublet-driven; §caveat + `doublet_overdispersion.html`) — and on real data the depth fix is fine.
...
capture depth**. Assignment asks, per pair, whether $N_{g,c}$ exceeds this null — the catch being that
the field must be estimated from a matrix that *contains the signal being detected* (see §The two
failure modes). Every method below is a different way to (a) estimate $\gamma_g,\kappa_c$ — cleanly or
not — and (b) test against the resulting null.
...
These are the failure modes of the **contingency strategy** our method belongs to — the one that pools
signal+noise into the margins and tests on a 2×2. (The mixture methods take the other strategy — they
model the latent signal/noise variable directly and so avoid both; full account, and why the 2×2 is the
contingency family's *native* object rather than a universal frame, in `literature_review`.) Both modes
are the same thing — signal leaking into the 2×2's *background* cells when the ambient null is estimated
from the pooled margins — landing in different cells and pushing the test opposite ways.

## Simpson's paradox — contaminated background → false positives (*precision*)

Even when guide $g$ is truly absent from cell $c$ and $g,c$ are noise-independent, signal from *other*
guides in *other* cells inflates the table's background and can flip the 2×2 odds ratio above 1 → a false
positive. fishash corrects it by replacing the "other-cells" counts with **noise-only** counts, estimated
by rank-1 Poisson completion of the signal-masked matrix.
...
## Self-masking — the focal cell's own signal → lost recall (*high MOI*)

The complementary leak lands in the *focal cell's own* background cell: cell $c$'s true guide(s) inflate
the "other guides in $c$" count — equivalently, its raw library size. A hypergeometric that uses that raw
library as the cell's exposure therefore expects too much ambient and **buries co-occurring real guides**
in the same cell. Harmless at low MOI (nothing else to miss), it bites hard at high MOI, where several
real guides mutually mask each other — precisely fishash's documented weak spot. The fix is to
decontaminate the cell margin (use the ambient depth). **CLEANSER already does exactly this** — its
cell-specific $L_i$ is a sub-threshold (≤2-count) ambient-depth estimate; **fishash does not** (it keeps
the raw library as its draws). This is the motivation for the recommended improvement (§below), and it is
a *recall* effect — the mirror image of Simpson's *precision* effect. Worked example there.

## A lesser third axis — overdispersion — and why we keep the plain hypergeometric

The hypergeometric assumes the ambient is *equidispersed*. But most apparent overdispersion is **mean
heterogeneity** — $\gamma_g$ spans 20×, $\kappa_c$ spans 50× — and the **conditional (Fisher) test
absorbs this for free** by conditioning on the margins (the sufficient statistics for the rank-1 rate).
This is *why* fishash is robust without a dispersion parameter, and why our marginal-NB attempt (which had
to estimate the rates *and* a dispersion) was fragile. What conditioning cannot absorb is **residual
clumping at a fixed mean** — and, as the experiments below show, that residual is small on real (cell-line
barnyard) ambient and not robustly estimable, so it does not warrant a dispersion parameter.
...
cell* — makes the cell margin the **ambient depth**. (CLEANSER reaches the same ambient-depth cell margin
by a different route — its sub-threshold $L_i$; our contribution is putting it inside fishash's
contingency core *alongside* the Simpson correction, so both the guide side and the cell side are cleaned
in one test.) Implemented modularly (`scripts/contingency_method.R`), it **reproduces fishash exactly**
with the observed margin, so the ambient-depth swap is a clean one-factor ablation.
...
The "comprehensive sim" below is the pre-framework apparatus (now in
`archive/old_comprehensive_sim_pipeline/`), superseded by the barnyard-anchored Model A / Model B
framework in `simulation_framework_report.qmd`. The artifact diagnosis it motivated — real ambient is
~Poisson — still stands and is why the framework anchors on real barnyard ambient.
:::
...
ground truth (full reproduction with code in `doublet_overdispersion.html`):

| chemistry | var/mean (index of dispersion) | max ambient count |
|---|---|---|
| **direct-capture** (before removing doublets) | 54 → ≈ 1.0 as purity tightens | 484 → ~3 |
| CROP-seq | **≈ 1.0** (Poisson) | 5 |

Real CROP-seq ambient is **Poisson** (var/mean ≈ 1.0, max 5). Direct-capture ambient is **conditionally
Poisson** — a bootstrap-calibrated goodness-of-fit finds the de-doubleted ambient indistinguishable from
$\mathrm{Poisson}(a_gd_c)$ (Pearson $\phi\approx0.99$, null band $[0.96,1.02]$; it rules out NB
$\alpha\gtrsim1$, is consistent with $\alpha\approx0$, cannot exclude mild $\alpha\lesssim0.25$) — but its
*raw* per-guide dispersion is **doublet-driven** (the
top 1% of cells by wrong-species load produce ~98.5% of the excess variance — a "human" cell with 484
mouse-guide UMIs is a doublet, not ambient) and only **collapses to Poisson** (var/mean → ~1.0, max 484 →
~3) once a gRNA-channel purity cut removes those doublets. So the trustworthy statement is that real,
de-doubleted ambient is ~Poisson in **both** chemistries. The comprehensive sim's ambient — the lab's sum-process
