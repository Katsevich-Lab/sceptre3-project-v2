...
# Warns on name-mismatch drops (so a renamed/sanitized id can't silently score
# as recall=0); aborts the dataset if drop fraction > 0.5.
suppressPackageStartupMessages({ library(Matrix) })
source(file.path(getwd(), "scripts", "sim_lib.R"))
OUT <- SIMFW()
man <- read.csv(file.path(OUT, "manifest_all.csv"))

calls_to_matrix <- function(f, gnames, cnames, method_lbl, id) {
  ...
  n_drop <- sum(!ok); frac_drop <- if (nrow(cl)) n_drop / nrow(cl) else 0
  if (n_drop) warning(sprintf("[%s/%s] %d/%d (%.1f%%) calls dropped (unknown guide/cell name)",
                              method_lbl, id, n_drop, nrow(cl), 100 * frac_drop), call. = FALSE)
  if (frac_drop > 0.5) stop(sprintf("[%s/%s] %.0f%% of calls have unmatched names -- aborting; check naming",
                                    method_lbl, id, 100 * frac_drop))
...
for (k in seq_len(nrow(man))) {
  ...
  for (mn in c("geomux", "crispat")) {
    A <- calls_to_matrix(file.path(d$dir, paste0(mn, "_calls.csv")),
                         rownames(counts), colnames(counts), method_lbl = mn, id = id)
    if (is.null(A)) next
