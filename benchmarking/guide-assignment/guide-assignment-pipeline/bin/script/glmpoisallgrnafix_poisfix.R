...
# Poisson mixture using the FIXED multiplicative M-step:
#   g_mus_pert1 = g_mus_pert0 * exp(curr_g_pert).
# Same as glmpoisgrnafix_poisfix but the offset also keeps the response covariates
...
assign_grnas_script <- function(response_matrix, grna_matrix, grna_target_df,
  ...
  run_allgrnafix_variant(
    ...
    cpus                   = cpus,
    family                 = "pois",
    fix_curr_g_pert_bug    = TRUE,    # multiplicative (fixed) M-step
