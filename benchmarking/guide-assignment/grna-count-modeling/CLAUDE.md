...
| **`method_comparison_2.qmd` → `.html`** | **Prose-narrative companion** to `method_comparison.qmd` (same three-method contrast, restructured as a landscape → method-contrast → synthesis story; no code chunks — static figures + inlined tables). Its four figures are regenerated by **`scripts/mc2_*.R`** (`mc2_dataset_landscape` / `mc2_variability_strips` / `mc2_guide_hist` / `mc2_concordance`; run order + inputs in `scripts/README_ambient_ceiling.md` §"method_comparison_2 figures"). The committed backing CSVs reproduce **byte-identically**. Content caveat carried in the doc's own notes: CROP-seq CLEANSER numbers use `--cs` only where regenerated (`a549`, `gastric_organoid`) — `run_cleanser_batch.sh` still hardcodes `--dc`; `mc2_cleanser_cs_calls.sh` produces the chemistry-correct calls. |
| **`fishash_repro.qmd` → `.html`** | **NEWEST — home-field simulation test + SCEPTRE head-to-head.** Runs Fishash+ Poisson (the depth fix in Poisson-tail form) through the **fishash authors' own `guidebender` simulations**, exact grids + exact per-entry scoring, our method overlaid on the paper's digitized field. Findings: adding the depth fix makes the contingency family the **best method for MOI 1–5** (F1 + full AUPRC), retains fishash's Simpson correction, and beats the current SCEPTRE mixture on accuracy while being **4×–1030× faster**; honest boundaries = a small low-MOI precision cost + over-call under non-Poisson (overdispersed) ambient. Backing suite: **`scripts/fishash_repro_*.R`** (14) + `results/fishash_repro/` (README = script→figure index; 443M dataset cache gitignored, byte-exact from the authors' seeds). Audited SOLID (24-agent adversarial review). |
...
├── method_comparison_2.{qmd,html}     # prose-narrative companion to method_comparison (figures via scripts/mc2_*.R)
├── fishash_repro.{qmd,html}           # NEWEST sim: Fishash+ Poisson through fishash's OWN guidebender sims + sceptre h2h (scripts/fishash_repro_*.R)
...
│                                      #   fishash_repro_*.R (14 — fishash home-field sims + sceptre h2h; backs fishash_repro.qmd)
...
│   ├── fishash_repro/                 # fishash_repro.qmd outputs (figures+CSVs+README; datasets/ 443M gitignored, byte-exact from the authors' seeds)
...
**Fishash home-field sims (current):** feeds `fishash_repro.qmd`. All scripts source
`scripts/fishash_repro_lib.R`; datasets regenerate byte-exact from the fishash authors' seeds
(443M cache gitignored under `results/fishash_repro/datasets/`). Run order (full script→figure
index in `results/fishash_repro/README.md`): `for s in varyMOI varyNguides varyNguidesLow
varyCorr; do fishash_repro_run.R $s; fishash_repro_plot.R $s; done` → `fishash_repro_fig6_overlay.R`
/ `_fig5_overlay.R` / `_auprc.R` / `_overdispersed.R` → `fishash_repro_sceptre_h2h.R` + `_h2h_reps.R`
+ the h2h plot scripts → `fishash_repro_endo_contrast.R` + `_barnyard_exo.R` → render
`fishash_repro.qmd`. Parity: `contingency_assign(cell_margin="observed",tail="hyper",fdr="GS")` ≡
`fishash(refit=10)` exactly; suite audited SOLID (24-agent adversarial review).
