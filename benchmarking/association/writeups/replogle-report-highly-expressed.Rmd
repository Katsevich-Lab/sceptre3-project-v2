...
get_genes_in_top_frac <- function(frac) {
  replogle_gene_summary_stats |>
    filter(gene_n_nonzero >= quantile(gene_n_nonzero, frac)) |>
    pull(gene)
}
...
results <- rbind(
  ...
  pass_qc_75 = gene %in% get_genes_in_top_frac(.75),
  pass_qc_50 = gene %in% get_genes_in_top_frac(.5),
  pass_qc_30 = gene %in% get_genes_in_top_frac(.3),
  pass_qc_20 = gene %in% get_genes_in_top_frac(.2),
...
qqs_50 = make_qq_plots(results, "pass_qc_50")
qqs_30 = make_qq_plots(results, "pass_qc_30")
qqs_20 = make_qq_plots(results, "pass_qc_20")
...
cowplot::plot_grid(plotlist = qqs_50, ncol = 1, labels = names(qqs_50))
cowplot::plot_grid(plotlist = qqs_30, ncol = 1, labels = names(qqs_30))
cowplot::plot_grid(plotlist = qqs_20, ncol = 1, labels = names(qqs_20))
...
```{r, echo=FALSE, fig.cap="ROC analysis",fig.show='hold', fig.align='center', out.width='100%'}
...
make_roc_plot <- function(results, qc_col) {
  ...
  p_roc <- ggplot(roc_data, aes(x = fpr, y = tpr, color = method_label)) +
    ...
    labs(
      title = bquote("Separation of pos. and neg. (QC =" ~ .(qc_col) *")"),
...
make_roc_plot(results, "pass_qc_75")
...
<!-- make_roc_plot(results, "pass_qc_trt") -->
<!-- ``` -->


<!-- ```{r,fig.show='hold', fig.align='center', out.width='100%', echo=FALSE} -->
<!-- make_roc_plot(results, "pass_qc_def") -->
<!-- ``` -->

```{r,fig.show='hold', fig.align='center', out.width='100%', echo=FALSE}
make_roc_plot(results, "pass_qc_50")
```


```{r,fig.show='hold', fig.align='center', out.width='100%', echo=FALSE}
make_roc_plot(results, "pass_qc_30")
```

```{r,fig.show='hold', fig.align='center', out.width='100%', echo=FALSE}
make_roc_plot(results, "pass_qc_20")
```
...
make_trt_zero_plots <- function(results, qc_col=NULL, cols_to_drop=NULL) {
  results_ = results
  if(!is.null(qc_col)) {
...
make_trt_zero_plots(results, "pass_qc_75", "p_value_frpert")
make_trt_zero_plots(results, "pass_qc_50", "p_value_frpert")
make_trt_zero_plots(results, "pass_qc_30", "p_value_frpert")
make_trt_zero_plots(results, "pass_qc_20", "p_value_frpert")
make_trt_zero_plots(results, cols_to_drop = "p_value_frpert")
...
make_fp_budget_plot <- function(
  ...
  y_var <- "tp"   # change to "tpr" if you want recall instead of counts
  ...
  p_tp_fp <- ggplot(plot_df, aes(x = fp_actual, y = .data[[y_var]], color = method)) +
    ...
    labs(
      title = paste0(if (y_var == "tp") "#TP recovered vs #FP allowed" else "TPR vs #FP allowed", " (QC = ", qc_col, ")"),
...
make_fp_budget_plot(results, "pass_qc_75")
make_fp_budget_plot(results, "pass_qc_50")
make_fp_budget_plot(results, "pass_qc_30")
make_fp_budget_plot(results, "pass_qc_20")
# make_fp_budget_plot(results, "pass_qc_trt")
# make_fp_budget_plot(results, "pass_qc_def")
...
make_bh_vs_pi_plot <- function(
  ...
  p_fdp_vs_pi_q01 <- ggplot(bench_sum, aes(x = pi, y = fdp_med, color = method, fill = method)) +
    ...
    labs(
      ...
      color = "Method",
      fill  = "Method",
      title = bquote("BH q = " * .(q_target) * " (N=" * .(N_total) * ", B=" * .(B) * ", QC =" ~ .(qc_col) * ")")
    ) +
    theme_bw(base_size = 14)
  p_fdp_vs_pi_q01
}
...
make_bh_vs_pi_plot(
  ...
  q_target = 0.1,
  N_total  = 50000,
  pi_grid  = c(0.001, 0.002, 0.005, .0075, 0.01)
)
make_bh_vs_pi_plot(
  results, "pass_qc_20",
  q_target = 0.1,
...
make_bh_pi_panel_plot <- function(
    results, qc_col,
    pi_grid,
    q_grid,
    N_total,
  ...
  df_long <- results %>%
    ...
    dplyr::select(pair_type, starts_with("p_value")) %>%
  ...
  p_fdp_pi <- ggplot(bench_sum, aes(x = q, y = fdp_med, color = method)) +
    ...
    labs(
      ...
      color = "Method",
      fill = "Method",
      title = bquote("BH FDP vs q (N =" ~ .(N_total/1000) * "k, QC =" ~ .(qc_col) *")")
    ) +
    theme_bw()
  p_fdp_pi
}

make_bh_pi_panel_plot(
  results,
  "pass_qc_75",
      pi_grid = c(0.001, 0.002, 0.005, .0075, 0.01, 0.015),
...
make_cor_df_replogle <- function(results, qc_col) {
  cor_mats = results %>%
    dplyr::filter(.data[[qc_col]]) %>%
    split(.$pair_type) |>
    lapply(function(df) df |> dplyr::select(p_value_scep, p_value_mix, p_value_frpert) |>
             cor(method="spearman", use = "complete.obs") |> as.data.frame())
  ...
  cor_results["qc"] = qc_col
  cor_results
}
bind_rows(make_cor_df_replogle(results, "pass_qc_75"),
make_cor_df_replogle(results, "pass_qc_trt"),
make_cor_df_replogle(results, "pass_qc_def"))
...
make_scatter_plots_replogle <- function(results, qc_col, num_negs=20000) {
  ...
  ggplot(pair_df, aes(x = x, y = y)) +
    ...
    theme_bw(base_size = 14) +
    ggtitle(paste0("Comparisons of p-values (QC =", qc_col, " )"))
}
make_scatter_plots_replogle(results, "pass_qc_75")
make_scatter_plots_replogle(results, "pass_qc_trt")
make_scatter_plots_replogle(results, "pass_qc_def")
...
make_on_target_sample_size_curve <- function(results, qc_col, dataset_name) {
  results_long <- results |>
    dplyr::filter(
      .data[[qc_col]], 
      pair_type == "positive control"
    ) |>
    dplyr::select(starts_with("p_value"), num_expressed_cells, gene) |>
    pivot_longer(
  ...
  ggplot(results_long, aes(x = num_expressed_cells, y = neglog10, color = method)) +
    ...
    labs(
      title = paste0(dataset_name, " on-targets: -log10(p-value) vs #cells (QC = ", qc_col, ")"),
...
make_on_target_sample_size_curve(results, "pass_qc_75", "Replogle")
make_on_target_sample_size_curve(results, "pass_qc_trt", "Replogle")
make_on_target_sample_size_curve(results, "pass_qc_def", "Replogle")
