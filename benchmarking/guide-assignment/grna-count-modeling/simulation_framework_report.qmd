...
**What the benchmark shows.** The **contingency family (depth_fix, fishash) and the
mixture family (crispat, SCEPTRE) control FDR and lead on accuracy** (depth_fix
0.854 / FDR 0.041 overall, fishash 0.806 / 0.027); the bare ambient test and Otsu
*over-call* (FDR 0.14–0.17); geomux *collapses on real-ambient direct-capture*.
**depth_fix's edge over fishash is specifically a high-MOI phenomenon** — Jaccard
gain +0.02 → +0.09 across MOI 1 → 15 on Model A, while on Model B's per-guide-fit
regimes (whose emergent MOI is mostly 1–3) the two are **essentially tied** at any
`d_sigma` (a `SIM_D_SIGMA=0.3` fairness sweep confirms this is not a model-structure
artifact). A fixed UMI cutoff is a strong baseline on easy data but its realized FDR
runs to ~0.5 on the hardest regimes — the "cutoff trap". Method rankings are stable
across purity; absolute direct-capture precision is purity-sensitive (real doublets
create a false-positive floor at 0.90 that vanishes at 0.99).
...
levels (chemistry-calibrated), and three MOI levels (low=1, high=5, vhigh=15). Because the ambient is real,
...
![Real vs simulated per-guide UMI histograms — one panel per regime; sim guide matched to real by above-valley mean. The ambient spike, the ambiguous middle, the signal-mode position, and the separation all reproduce.](results/sim_framework/regime_histograms_all.png)
...
**High MOI is where depth_fix beats fishash, specifically.** On Model A (real
barnyard ambient) at controlled MOI, depth_fix's Jaccard advantage over fishash
grows monotonically with MOI — co-occurring real guides inflate fishash's
library margin, and the ambient-depth margin recovers the recall:

```{r moicmp}
mc <- rd("depthfix_vs_fishash_moi.csv")
if (!is.null(mc)) {
  w <- mc %>% pivot_wider(names_from = method, values_from = c(jaccard, recall))
  kbl(w, caption = "depth_fix vs fishash by MOI (Model A: low=1, high=5, vhigh=15). The Jaccard gain grows from +0.02 to +0.09 as fishash's recall drops from 0.85 to 0.76 under co-occurrence.")
}
```

**On Model B's per-guide-fit regimes, depth_fix and fishash are essentially
tied** (overall Jaccard ~0.88 each; fishash actually has marginally tighter FDR).
That is consistent with the masking story: most real datasets have low emergent
MOI (1–3 guides/cell after deflating signal_frac for ambient leakage), so the
co-occurrence regime where depth_fix wins doesn't arise.  A `SIM_D_SIGMA=0.3`
fairness sweep (vs 0.05 default) confirms this is not a structural artifact:
the depth_fix–fishash gap on Model B is ≈ 0 at both `d_sigma` settings, with
per-regime shifts < 0.02 across all 13 regimes.

```{r dsigma}
ds <- rd("dsigma_sweep.csv")
if (!is.null(ds)) {
  kbl(ds %>% select(regime, d_sigma, gap_jac) %>%
        pivot_wider(names_from = d_sigma, values_from = gap_jac, names_prefix = "d_sigma_") %>%
        mutate(shift = round(d_sigma_0.3 - d_sigma_0.05, 3),
               d_sigma_0.05 = round(d_sigma_0.05, 3),
               d_sigma_0.3 = round(d_sigma_0.3, 3)) %>%
        arrange(d_sigma_0.05),
      caption = "Per-regime depth_fix minus fishash Jaccard on Model B at d_sigma = 0.05 (primary) vs 0.30 (fairness sweep). Tiny shifts confirm the absence of a depth_fix advantage on these regimes is not a model-fairness artifact.")
}
```

![Per-regime depth_fix – fishash Jaccard gap at d_sigma = 0.05 vs 0.3 on the 13 Model B regimes (fairness sweep). Bars are essentially equal; the gap (or lack thereof) is robust to the d_sigma assumption.](results/sim_framework/fig_dsigma_sweep.png)
...
- **Contingency family (depth-fix, fishash).** Lead on Jaccard while controlling
  FDR.  On the per-guide-fit Model B regimes the two are essentially tied;
  depth-fix's edge is specifically a **high-MOI Model A** phenomenon (Jaccard gain
  +0.02 → +0.09 as MOI goes 1 → 15), where co-occurring real guides inflate
  fishash's library margin and the ambient-depth margin recovers recall.
...
- **geomux.** Faithfully reproduces its known **collapse on direct-capture**: its
  ...
  direct-capture and on the heaviest-ambient Model B regimes.
...
5. **The depth_fix advantage is scoped to high MOI, not Model B regimes.** On Model
   A at MOI 1, depth_fix and fishash are within 0.02 Jaccard; the gap grows to
   +0.09 at MOI 15 (the masking effect).  On the per-guide-fit Model B regimes
   (real signal_frac, mostly emergent MOI 1–3), the two are essentially tied — and
   a `SIM_D_SIGMA=0.3` fairness sweep confirms this is not a structural artifact.
   The honest framing: depth_fix is the **safer default at high MOI**; fishash
   slightly edges it on FDR control at low MOI.
...
```{r provenance}
cat("Code (in pipeline order, all under scripts/):",
    "sim_lib.R (registry, scoring, realism gate), barnyard_io.R (authors' QC),",
    "contingency_method.R (depth_fix), sim_modelA.R, sim_characterize.R,",
    "sim_modelB.R, sim_regimes.R, sim_realism_gate.R, sim_run_methods.R,",
    "run_geomux_sims.py (.venv_geomux_v5), run_crispat_sims.py (.venv_crispat),",
    "sim_score_external.R, sim_analysis.R, sim_validate_hist.R,",
    "sim_plot_real_distributions.R, sim_plot_within_regime.R, sim_poc_paired.R,",
    "sim_dsigma_sweep.R + sim_dsigma_compare.R (the SIM_D_SIGMA=0.3 fairness sweep).\n")
```
