...
p1 <- ggplot(pgA, aes(mean, var, colour = chem)) +
  ...
  labs(x = "per-guide mean ambient UMIs (log)", y = "per-guide variance (log)", colour = NULL) +
...
p2 <- ggplot(data.frame(mf = mf, region = region), aes(mf, fill = region)) +
  ...
  annotate("text", x = 0.10, y = Inf, label = "old gate 0.90\n(mouse frac 0.10)", vjust = 1.4, hjust = -0.06, size = 4.5, colour = "grey35") +
  annotate("text", x = 0.01, y = Inf, label = "new gate 0.99\n(mouse frac 0.01)", vjust = 1.4, hjust = 1.06, size = 4.5, colour = "firebrick") +
  ...
  labs(x = "fraction of GEX UMIs that are mouse   mus / (homo + mus)   (log)", y = "cells (sqrt)", fill = NULL) +
  guides(fill = guide_legend(nrow = 2)) +
  theme_bw(base_size = 16) + theme(legend.position = "top", legend.text = element_text(size = 13))
ggsave(file.path(OUT, "barnyard_frac_mouse.png"), p2, width = 9.5, height = 5.2, dpi = 130)

## ---- 3. per-guide var vs mean: 0.90 gate -> 0.99 gate ----------------------------
# Tightening the GEX-purity gate 0.90 -> 0.99 removes most of the ambient over-dispersion.
# (The non-circular "isolated single-cell spikes" check now lives in barnyard_five_cells.R,
# the global-5-cells figure below.)
...
ggsave(file.path(OUT, "barnyard_vmr_threshold.png"), p3, width = 9, height = 4.6, dpi = 130)
...
cat(sprintf("per-guide median var/mean: 0.90 -> %.1f ; 0.99 -> %.1f\n",
            medV["GEX gate 0.90"], medV["GEX gate 0.99"]))
